...
class DoublyLinkedList {
   ...
   */
  insert (data) {
    const newNode = DoublyLinkedList.createNewNode(data);
    if (this.isEmpty()) {
  ...
  insertBefore (nodeData, dataToInsert) {
    return this.insertAt(this.indexOf(nodeData), dataToInsert);
   ...
   */
  removeAt (index) {
    const nodeToRemove = this.findAt(index);
   ...
   */
  removeNode (nodeData) {
    return this.removeAt(this.indexOf(nodeData));
  }
   ...
   */
  contains (nodeData) {
    return this.indexOf(nodeData) > -1;
  }
